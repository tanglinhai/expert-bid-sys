...
<script>
    ...
    export default {
        ...
        mounted() {
            this.init();
            if(this.which_one_show==1){//1：解锁的那个页面显示，反之进度条的那个显示
                $(".pro_table").show();
                $(".unlock_table").hide();
            }else{
                $(".unlock_table").show();
            ...
            init() {   //初始化 table的数据
                ...
                this.$axios.post('/api/table_data','post',{//通过包id
                    ...
                    if (res.status === 200) {
                        ...
                        this.which_one_show=res.data.bidMsg.eviewrItemsMsg.isShow;
                        console.log(this.which_one_show);

                        // var a=true;
                        // var arrA=[];
                        // this.msgBox.forEach((m,i)=>{
                        //     console.log(m, i);
                        //     if(m.dates==='100'){
                        //         a=false;
                        //         arrA.push(a)
                        ...
                        // });
                        // console.log(arrA.length);
                        // if(this.msgBox.length==arrA.length||this.msgBox.length!==0){
                        //     $(".unlock_table").show();
                        //     $(".pro_table").hide();
                        //
                        // }else{
